...
def Simulation(initial=5, timesteps=400, maximum=100):
	...
	for t in xrange(timesteps):
		...
		t1=time.clock()
		AddList = RollingLinks(ProbM,t,5*len(Nodes))
		for L in AddList:
...
if __name__=="__main__":
	...
	plt.show()
	n,bins,schrott , = plt.hist(RankoneTimes, label = "Time spent at Rank one")
	print bins
	plt.ylabel("Time spent at Rank one")
	plt.show()
	n,bins,schrott , = plt.hist(TimetoOnes, label = "Time it takes to reach Rank one")
	print bins
